...
pub enum TrackError {
    ...
    Overlap { clip_id: crate::core::clip::ClipId },
}
...
impl Track {
    ...
    /// Returns Ok(()) if successful, Err(TrackError::Overlap) if the clip overlaps with existing clips
    /// Maintains sorted order by timeline_start
    ...
    /// Add a clip to the track without overlap validation (unsafe operation)
    /// Use this only when you've already validated overlaps externally
    pub fn add_clip_unchecked(&mut self, clip: Clip) {
        self.clips.push(clip);
        self.clips.sort_by_key(|c| c.timeline_start);
    }

    /// Remove a clip by ID
    pub fn remove_clip(&mut self, clip_id: crate::core::clip::ClipId) -> Option<Clip> {
    ...
    /// Get the duration of the track (end of last clip) in nanoseconds
